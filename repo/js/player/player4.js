	...
	if (videoid != 'undefined' && videoid != undefined && videoid != '') {
		...
		player = videojs(videoid, {
			...
			if (typeof player.ima == 'function') {
				player.ima({ vastLoadTimeout: 10000, adTagUrl: '' });   
				
				console.log('first preroll ads');
				await loadScriptAsync('https://ads.blogherads.com/static/blogherads.js');
				await loadScriptAsync('https://ads.blogherads.com/sk/12/124/1242403/31822/header.js');
			...
			function common(callback) {
				...
				let isSheLoaded=false;
				player.on("play", async function (e) {
			...
			player.on('ads-manager', function (response) {
				...
				adsManager.addEventListener(google.ima.AdEvent.Type.ALL_ADS_COMPLETED, function () {
					...
					/*
					if (postrollIndex < postrollAds && isVideoEnded) {
						postrollIndex++;
						intializeMidrollAds()
						// window.pribidInit('midroll');
					...
					if (res && res.length > 3) {
						...
						if (resp.status == 1) {
							...
							resData.forEach(function (val) {
								...
								let href_post_id = base_url + `embedcv/` + val.post_id;
...
async function loadScriptAsync(scriptUrl) {
    return new Promise((resolve, reject) => {
        ...
        // Resolve when script is successfully loaded
